...
int l1c_get_perm_macaddr(struct alx_hw *hw, u8 *addr)
{
	...
	mac0 = alx_read_mem32(hw, L1C_STAD0);
	mac1 = alx_read_mem32(hw, L1C_STAD1);
	...
	*(__be16 *)addr = cpu_to_be16((u16)mac1);
	...
	val = alx_read_mem32(hw, L1C_TWSI_DBG);
	otp_ctrl = alx_read_mem32(hw, L1C_EFUSE_CTRL2);
	otp_flag = alx_read_mem32(hw, L1C_MASTER);

	if ((val & L1C_TWSI_DBG_DEV_EXIST) != 0 ||
	    (otp_flag & L1C_MASTER_OTP_FLG) != 0) {
		/* nov-memory exist, do software-autoload */
		/* enable OTP_CLK for L1C */
		if (hw->pdev->device == L1C_DEV_ID ||
		    hw->pdev->device == L2C_DEV_ID) {
			if ((otp_ctrl & L1C_EFUSE_CTRL2_CLK_EN) != 0) {
				alx_write_mem32(hw, L1C_EFUSE_CTRL2,
					    otp_ctrl | L1C_EFUSE_CTRL2_CLK_EN);
				udelay(5);
			}
		}
		/* raise voltage temporally for L2CB/L1D */
		if (hw->pdev->device == L2CB_DEV_ID ||
		    hw->pdev->device == L2CB2_DEV_ID) {
		...
		/* do load */
		val = alx_read_mem32(hw, L1C_SLD);
		alx_write_mem32(hw, L1C_SLD, val | L1C_SLD_START);
...
int l1c_reset_mac(struct alx_hw *hw)
{
	...
	alx_write_mem32(hw, L1C_IMR, 0);
	alx_write_mem32(hw, L1C_ISR, L1C_ISR_DIS);

	ret = l1c_enable_mac(hw, false, 0);
	if (ret != 0)
		return ret;
	/* reset whole mac safely. OOB is meaningful for L1D only  */
	mrst_val = alx_read_mem32(hw, L1C_MASTER);
	mrst_val |= L1C_MASTER_OOB_DIS;
	alx_write_mem32(hw, L1C_MASTER, mrst_val | L1C_MASTER_DMA_MAC_RST);

	/* make sure it's idle */
	for (i = 0; i < L1C_DMA_MAC_RST_TO; i++) {
		val = alx_read_mem32(hw, L1C_MASTER);
	...
	val = alx_read_mem32(hw, L1C_MAC_CTRL);
	alx_write_mem32(hw, L1C_MAC_CTRL, val | L1C_MAC_CTRL_WOLSPED_SWEN);

	/* clk switch setting */
	val = alx_read_mem32(hw, L1C_SERDES);
	switch (hw->pdev->device) {
	case L2CB_DEV_ID:
		alx_write_mem32(hw, L1C_SERDES, val & ~L1C_SERDES_PHYCLK_SLWDWN);
		break;
	case L2CB2_DEV_ID:
	case L1D2_DEV_ID:
		alx_write_mem32(hw, L1C_SERDES,
...
int l1c_reset_phy(struct alx_hw *hw, bool pws_en, bool az_en, bool ptp_en)
{
	...
	/* reset PHY core */
	val = alx_read_mem32(hw, L1C_PHY_CTRL);
	...
	alx_write_mem32(hw, L1C_PHY_CTRL, val);
	udelay(10); /* 5us is enough */
	alx_write_mem32(hw, L1C_PHY_CTRL, val | L1C_PHY_CTRL_DSPRST_OUT);
	...
	if (hw->pdev->device == L2CB_DEV_ID || hw->pdev->device == L2CB2_DEV_ID) {
	...
	if (pws_en) {
		...
		l1c_write_phydbg(hw, true, L1C_MIIDBG_LEGCYPS,
				 (hw->pdev->device == L1D_DEV_ID ||
				  hw->pdev->device == L1D2_DEV_ID) ?
	...
	if (hw->pdev->device == L1D_DEV_ID ||
	    hw->pdev->device == L1D2_DEV_ID ||
	    hw->pdev->device == L2CB2_DEV_ID) {
		if (az_en) {
			switch (hw->pdev->device) {
			case L2CB2_DEV_ID:
				alx_write_mem32(hw, L1C_LPI_DECISN_TIMER,
			...
			val = alx_read_mem32(hw, L1C_LPI_CTRL);
			alx_write_mem32(hw, L1C_LPI_CTRL, val & ~L1C_LPI_CTRL_EN);
			l1c_write_phy(hw, true, L1C_MIIEXT_ANEG, true,
...
int l1c_reset_pcie(struct alx_hw *hw, bool l0s_en, bool l1_en)
{
	...
	val = alx_read_mem32(hw, L1C_LTSSM_CTRL);
	alx_write_mem32(hw, L1C_LTSSM_CTRL, val & ~L1C_LTSSM_WRO_EN);

	/* mask some pcie error bits */
	val = alx_read_mem32(hw, L1C_UE_SVRT);
	val &= ~(L1C_UE_SVRT_DLPROTERR | L1C_UE_SVRT_FCPROTERR);
	alx_write_mem32(hw, L1C_UE_SVRT, val);

	/* pclk */
	val = alx_read_mem32(hw, L1C_MASTER);
	val &= ~L1C_MASTER_PCLKSEL_SRDS;
	alx_write_mem32(hw, L1C_MASTER, val);

	/* Set 1000 bit 2, only used for L1c/L2c , WOL usage */
	if (hw->pdev->device == L1C_DEV_ID || hw->pdev->device == L2C_DEV_ID) {
		val = alx_read_mem32(hw, L1C_PPHY_MISC1);
		alx_write_mem32(hw, L1C_PPHY_MISC1, val | L1C_PPHY_MISC1_RCVDET);
	} else { /* other device should set bit 5 of reg1400 for WOL */
		if ((val & L1C_MASTER_WAKEN_25M) == 0)
			alx_write_mem32(hw, L1C_MASTER, val | L1C_MASTER_WAKEN_25M);
	}
	/* l2cb 1.0*/
	if (hw->pdev->device == L2CB_DEV_ID && hw->pdev->revision == L2CB_V10) {
		val = alx_read_mem32(hw, L1C_PPHY_MISC2);
		...
		alx_write_mem32(hw, L1C_PPHY_MISC2, val);
		/* extend L1 sync timer, this will use more power,
		 * only for L2cb v1.0*/
		if (!hw->aps_en) {
			val = alx_read_mem32(hw, L1C_LNK_CTRL);
			alx_write_mem32(hw, L1C_LNK_CTRL,
				    val | L1C_LNK_CTRL_EXTSYNC);
		}
	}

	/* l2cbv1.x & l1dv1.x */
	if (hw->pdev->device == L2CB_DEV_ID || hw->pdev->device == L1D_DEV_ID) {
		val = alx_read_mem32(hw, L1C_PMCTRL);
		alx_write_mem32(hw, L1C_PMCTRL, val | L1C_PMCTRL_L0S_BUFSRX_EN);
		/* clear vendor message for L1d & L2cb */
		val = alx_read_mem32(hw, L1C_DMA_DBG);
		alx_write_mem32(hw, L1C_DMA_DBG, val & ~L1C_DMA_DBG_VENDOR_MSG);
	}
	...
	if (hw->hi_txperf) {
		...
		alx_write_mem32(hw, L1C_PPHY_MISC1, val);
	}
	/* l0s, l1 setting */
...
int l1c_enable_mac(struct alx_hw *hw, bool en, u16 en_ctrl)
{
  ...
  u32 ctrl_hw_map[] = {
    ...
    rxq = alx_read_mem32(hw, L1C_RXQ0);
	txq = alx_read_mem32(hw, L1C_TXQ0);
    mac = alx_read_mem32(hw, L1C_MAC_CTRL);

	if (en) { /* enable */
		alx_write_mem32(hw, L1C_RXQ0, rxq | L1C_RXQ0_EN);
		alx_write_mem32(hw, L1C_TXQ0, txq | L1C_TXQ0_EN);
    ...
    alx_write_mem32(hw, L1C_MAC_CTRL, mac | L1C_MAC_CTRL_TX_EN);
	} else { /* disable mac */
		alx_write_mem32(hw, L1C_RXQ0, rxq & ~L1C_RXQ0_EN);
		alx_write_mem32(hw, L1C_TXQ0, txq & ~L1C_TXQ0_EN);
		...
		for (i = 0; i < L1C_DMA_MAC_RST_TO; i++) {/* wait atmost 1ms */
			val = alx_read_mem32(hw, L1C_MAC_STS);
		...
		if (L1C_DMA_MAC_RST_TO == i)
			return LX_ERR_RSTMAC;
		/* stop mac tx/rx */
		alx_write_mem32(hw, L1C_MAC_CTRL,
...
int l1c_enable_aspm(struct alx_hw *hw, bool l0s_en, bool l1_en, u8 lnk_stat)
{
	...
	pmctrl = alx_read_mem32(hw, L1C_PMCTRL);
	...
	/* l1 timer */
	if (hw->pdev->device == L2CB2_DEV_ID || hw->pdev->device == L1D2_DEV_ID) {
	...
	if (hw->pdev->device == L2CB_DEV_ID || hw->pdev->device == L1D_DEV_ID ||
	    hw->pdev->device == L2CB2_DEV_ID || hw->pdev->device == L1D2_DEV_ID) {
		...
		if (!linkon ||
		    (!hw->aps_en && hw->pdev->device == L2CB_DEV_ID)) {
			pmctrl &= ~L1C_PMCTRL_L0S_EN;
		}
	} else { /* l1c */
	...
	alx_write_mem32(hw, L1C_PMCTRL, pmctrl);
		       ...
		       u8 lnk_cap, bool fc_en)
{
	...
	alx_write_mem32(hw, L1C_DRV, val);
...
{
	u16 phy_val;
	bool adj_th = (hw->pdev->device == L2CB_DEV_ID ||
			hw->pdev->device == L2CB2_DEV_ID ||
			hw->pdev->device == L1D_DEV_ID ||
			hw->pdev->device == L1D2_DEV_ID) ? true : false;

	if (linkon) {
		/* az with broadcom, Half-amp */
		if (hw->pdev->device == L1D2_DEV_ID) {
		    ...
		    bool mac_txen, bool mac_rxen, bool pws_en)
{
	...
	master_ctrl = alx_read_mem32(hw, L1C_MASTER);
	master_ctrl &= ~L1C_MASTER_PCLKSEL_SRDS;

	mac_ctrl = alx_read_mem32(hw, L1C_MAC_CTRL);
	/* 10/100 half */
	...
	phy_ctrl = alx_read_mem32(hw, L1C_PHY_CTRL);
	...
	alx_write_mem32(hw, L1C_MASTER, master_ctrl);
	alx_write_mem32(hw, L1C_MAC_CTRL, mac_ctrl);
	alx_write_mem32(hw, L1C_PHY_CTRL, phy_ctrl);
		 ...
		 u16 reg, u16 *data)
{
	...
	*data = 0;
	...
	if (ext) {
		...
		alx_write_mem32(hw, L1C_MDIO_EXTN, val);
	...
	alx_write_mem32(hw, L1C_MDIO, val);

	for (i = 0; i < L1C_MDIO_MAX_AC_TO; i++) {
		val = alx_read_mem32(hw, L1C_MDIO);
		 ...
		 u16 mtu, u16 int_mod, bool hash_legacy)
{
	...
	alx_write_mem32(hw, L1C_STAD0, val);
	val = be16_to_cpu(*(__be16 *)addr);
	alx_write_mem32(hw, L1C_STAD1, val);

	/* clear multicast hash table, algrithm */
	alx_write_mem32(hw, L1C_HASH_TBL0, 0);
	alx_write_mem32(hw, L1C_HASH_TBL1, 0);
	val = alx_read_mem32(hw, L1C_MAC_CTRL);
	if (hash_legacy)
		val |= L1C_MAC_CTRL_MHASH_ALG_HI5B;
	else
		val &= ~L1C_MAC_CTRL_MHASH_ALG_HI5B;
	alx_write_mem32(hw, L1C_MAC_CTRL, val);

	/* clear any wol setting/status */
	val = alx_read_mem32(hw, L1C_WOL0);
	alx_write_mem32(hw, L1C_WOL0, 0);

	/* clk gating */
	alx_write_mem32(hw, L1C_CLK_GATE, (hw->pdev->device == L1D_DEV_ID) ? 0 :
		       (L1C_CLK_GATE_DMAR | L1C_CLK_GATE_DMAW |
			L1C_CLK_GATE_TXQ  | L1C_CLK_GATE_RXQ  |
			L1C_CLK_GATE_TXMAC));

	/* descriptor ring base memory */
	alx_write_mem32(hw, L1C_TX_BASE_ADDR_HI, txmem_hi);
	alx_write_mem32(hw, L1C_TPD_RING_SZ, txring_sz);
	switch (tx_qnum) {
	case 2:
		alx_write_mem32(hw, L1C_TPD_PRI1_ADDR_LO, tx_mem_lo[1]);
		/* fall through */
	case 1:
		alx_write_mem32(hw, L1C_TPD_PRI0_ADDR_LO, tx_mem_lo[0]);
		break;
	default:
		return LX_ERR_PARM;
	}
	alx_write_mem32(hw, L1C_RX_BASE_ADDR_HI, rxmem_hi);
	alx_write_mem32(hw, L1C_RFD_ADDR_LO, rfdmem_lo);
	alx_write_mem32(hw, L1C_RRD_ADDR_LO, rrdmem_lo);
	alx_write_mem32(hw, L1C_RFD_BUF_SZ, rxbuf_sz);
	alx_write_mem32(hw, L1C_RRD_RING_SZ, rxring_sz);
	alx_write_mem32(hw, L1C_RFD_RING_SZ, rxring_sz);
	alx_write_mem32(hw, L1C_SMB_TIMER, smb_timer * 500UL);

	if (hw->pdev->device == L2CB_DEV_ID) {
		/* revise SRAM configuration */
		alx_write_mem32(hw, L1C_SRAM5, L1C_SRAM_RXF_LEN_L2CB1);
		alx_write_mem32(hw, L1C_SRAM7, L1C_SRAM_TXF_LEN_L2CB1);
		alx_write_mem32(hw, L1C_SRAM4, L1C_SRAM_RXF_HT_L2CB1);
		alx_write_mem32(hw, L1C_SRAM0, L1C_SRAM_RFD_HT_L2CB1);
		alx_write_mem32(hw, L1C_SRAM6, L1C_SRAM_TXF_HT_L2CB1);
		alx_write_mem32(hw, L1C_SRAM2, L1C_SRAM_TRD_HT_L2CB1);
		alx_write_mem32(hw, L1C_TXQ2, 0); /* TX watermark, goto L1 state.*/
		alx_write_mem32(hw, L1C_RXQ3, 0); /* RXD threshold. */
	}
	alx_write_mem32(hw, L1C_SRAM9, L1C_SRAM_LOAD_PTR);
	...
	alx_write_mem32(hw, L1C_IRQ_MODU_TIMER,
		    FIELDL(L1C_IRQ_MODU_TIMER1, int_mod) |
		    FIELDL(L1C_IRQ_MODU_TIMER2, int_mod >> 1));

	/* tpd threshold to trig int */
	alx_write_mem32(hw, L1C_TINT_TPD_THRSHLD, (u32)txring_sz / 3);
	alx_write_mem32(hw, L1C_TINT_TIMER, int_mod * 2);
	/* re-send int */
	alx_write_mem32(hw, L1C_INT_RETRIG, L1C_INT_RETRIG_TO);

	/* mtu */
	alx_write_mem32(hw, L1C_MTU, (u32)(mtu + 4 + 4)); /* crc + vlan */
	...
	alx_write_mem32(hw, L1C_TXQ1, val >> 3);

	val = alx_read_mem32(hw, L1C_DEV_CTRL);
	...
	if (dmar_len < L1C_DEV_CTRL_MAXRRS_MIN) {
		...
		alx_write_mem32(hw, L1C_DEV_CTRL, val);
	...
	alx_write_mem32(hw, L1C_TXQ0, val);

	/* fc */
	val = alx_read_mem32(hw, L1C_SRAM5);
	...
	if ((hw->pdev->device & 1) != 0) {
		FIELD_SETL(val, L1C_RXQ0_ASPM_THRESH,
			   (hw->pdev->device == L1D2_DEV_ID) ?
			   L1C_RXQ0_ASPM_THRESH_NO :
			   L1C_RXQ0_ASPM_THRESH_100M);
	}
	alx_write_mem32(hw, L1C_RXQ0, val);

	/* rfd producer index */
	alx_write_mem32(hw, L1C_RFD_PIDX, (u32)rxring_sz - 1);
	...
	alx_write_mem32(hw, L1C_DMA, val);
